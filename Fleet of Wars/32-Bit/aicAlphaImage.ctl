...
'       ROUTINES REMOVED. FOR THE COMPLETE UP TO DATE VERSIONS OF THESE CLASSES VISIT:
'       http://www.planetsourcecode.com/vb/scripts/ShowCode.asp?txtCodeId=67466&lngWId=1

' See the Usage.RTF file provided for more information
' Last updated: 7 Oct 2007
'   added ICrossTalk, cCrossComm classes for better usercontrol-usercontrol communication
'   added GDI+ IDE-safe thunk routine in cCrossComm
'   reworked zb_AddressOf to reduce overall memory requirements
...
' GDIplusEnabled. Returns true if the system can use GDI+
' GDIplusInitialized. Returns true if the usercontol started GDI+
...
' IntensityOffset.  Lightens or darkens image when rendered
...
' Rotation. Determines angle of rotation (run time only). RotateEnabled must be True for actual rotation
' RotateEnabled. Boolean whether image will rotate or not.
